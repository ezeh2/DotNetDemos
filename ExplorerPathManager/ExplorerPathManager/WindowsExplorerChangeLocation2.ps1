...
# WindowsExplorerChangeLocation2.ps1 opens Windows File Explorer and shows directory $path
# But it opens a new instance if there is not already instanece runnig with the right path

# source: https://stackoverflow.com/questions/46658314/select-specific-files-in-explorer-with-powershell
...
Add-Content -Path "$PSScriptRoot\WindowsExplorerChangeLocationPaths.txt" -Value $dt" "$path
...
namespace SHOpenFolderAndSelectItems
{
    ...
    static public class ShowSelectedInExplorer
    {
        [Flags]
        enum SHCONT : ushort
        {
...
# add individual folders here (unused)
$list.Add($path)
#$list.Add("setup")
#$list.Add("Fonts")

[SHOpenFolderAndSelectItems.ShowSelectedInExplorer]::FileOrFolder($path);
